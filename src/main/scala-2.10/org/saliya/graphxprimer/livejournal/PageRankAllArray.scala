...
object PageRankAllArray {
                                                                  ...
                                                                  srcId: Option[VertexId] = None): Graph[Double, Double] =
  {
    ...
    def sendMessage(edge: EdgeTriplet[(Double, Array[Double]), Double]): Iterator[(VertexId, Array[Double])] = {
      if (edge.srcAttr._2(0) > tol) {
        Iterator((edge.dstId, createArray(edge.srcAttr._2(0) * edge.attr)))
      } else {
        Iterator.empty
      }
    }
