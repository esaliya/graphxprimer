...
/**
  ...
  */
object ProgramNoCommSimpleVertex {
...
*/
  ...
  def vprogWrapper(k: Int, random: java.util.Random, fieldSize: Int, gf: GaloisField) =
    (vertexId: VertexId, value: (Int, Int), message: Int) => {
    ...
    if (message != 0) {
      ...
      for (j <- 1 until i) {
        for (neighbor <- 0 until neighbors) {
          val weight = random.nextInt(fieldSize)
          var product = gf.multiply(i, i) // dummy multiplication
          product = gf.multiply(weight, product)
          result = gf.add(i, product)
        }
      }
  ...
  def sendMsg(triplet: EdgeTriplet[(Int, Int), Int]):
...
//    val edges = new Array[Edge[Int]](n-1) // this is one way directed graph
  ...
  // Change the vertex signature to have just color (Int) and another Int to keep track of iteration
  def createGraphFromFile(f:String, k: Int, n: Int, sc: SparkContext): (Graph[(Int, Int), Int], Int) ={
    ...
    for (line <- Source.fromFile(f).getLines()){
      ...
      if (!line.startsWith("#") && mode != 2){
        ...
        if (mode == 0){
          ...
          colors.add(color)
          vertices(vertexId) = (vertexId.toLong, (color, 0))
        } else if (mode == 1){
